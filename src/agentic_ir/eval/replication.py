...
def render_table(pairs: Sequence[ReplicationPair]) -> str:
    ...
    caption = (
        ...
        r"drew a different evidence pool, locating the divergence in retrieval "
        r"rather than in generation. One cause is the \texttt{PYTHONHASHSEED} "
        r"column: the evidence pool is assembled from sets, whose iteration "
        r"order follows Python's per-process string hashing unless the variable "
        r"is set. This is the report's own error bar on execution, separate from "
        r"every bootstrap interval elsewhere, which resample questions and hold "
        r"the execution fixed. An effect smaller than this drift is not "
        r"established by a single run."
    )
