...
is complete, \texttt{agentic\_no\_planner} is mid-run and
\texttt{agentic\_no\_kg} had not started. The verifier ablation is therefore the only one measured
twice, and the two measurements disagree --- which is precisely why the
planner's and the graph's single nulls are not treated here as settled either.
A component that looked significant on HotpotQA turned out to do nothing on
2WikiMultihopQA; the two that looked like nulls on HotpotQA have not yet had
the chance to do the reverse.
