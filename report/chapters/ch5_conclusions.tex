...
will answer, and the graph ablation on both datasets remains to be reported.
...
full-system configurations are complete on both datasets,
\texttt{agentic\_no\_verifier} and \texttt{agentic\_no\_planner} are complete
on HotpotQA and not started on 2WikiMultihopQA, and \texttt{agentic\_no\_kg}
is in flight on HotpotQA, short of 250 when the tables were last generated,
and not started on 2WikiMultihopQA. The backward
edge's contribution is therefore a one-dataset number, the planner's is a
one-dataset null, and the graph's is absent; each is marked so where it is
due.
