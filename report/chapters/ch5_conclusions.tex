...
a cap of 20 and a measured mean of 4.26 on HotpotQA and 3.80 on
2WikiMultihopQA, since tool routing, entity linking,
...
structure: the agentic system as designed is not the most accurate system in
this study and is the only one that can show its work --- but the ablation that
deletes its first component is both, beating the strongest baseline on answer
quality while grounding nine citations in ten. The trade the report set out to
describe turns out to be a trade the architecture imposed on itself.
...
HotpotQA and 0.421 against 0.327 on 2WikiMultihopQA, on neither of which is a
significance mark placed: the two rows are scored under different protocols,
and the like-for-like column is Pool SP-R, where the lead is significant on
both datasets. The generative baselines were reported in an earlier
...
to test, it is measured rather than asserted, and on HotpotQA it lands in the
direction the design predicted.

On 2WikiMultihopQA it does not. The same ablation over that dataset's 250
questions scores an exact match of 0.216 and F1 of 0.266 against the full
system's 0.224 and 0.267: a paired $\Delta$F1 of $-0.001$, $[-0.025, +0.025]$,
$p = 1.000$. The loop was not idle there --- it ran on 98 of the 250 questions
and a later cycle was selected on 64 --- and it charged the same price, 3.80
model calls against 2.03 and a median of 35.4\,s against 18.4\,s. It simply
bought nothing the evaluation can detect. Chapter~\ref{ch:implementation}
gives the two readings the grid permits: that a system at F1 0.267 is failing
for reasons a second attempt at the same corpus cannot repair, and that
2WikiMultihopQA's comparison templates defeat an entailment check that reads
one answer sentence. The ablated system being \emph{better} attributed there
--- SP-F1 0.465 against 0.421 --- supports the second.

So the finding is conditional, and this report states it conditionally: the
backward edge helps on HotpotQA and not on 2WikiMultihopQA. The report's own
standard is that one dataset does not settle a direction, and having now run
the second, the honest conclusion is that the two disagree. It is also a
statement about the edge, not about the system: a verifier that adds four
points on one dataset to an answer accuracy that is itself nine points behind
\texttt{self\_ask} has proved that feedback can help, not that this system is
the one to deploy.
...
2WikiMultihopQA answered that question against the Planner, and it is the
most consequential result in this study. There
\texttt{agentic\_no\_planner} scores an exact match of 0.324 and F1 of 0.388
against the full system's 0.224 and 0.267: $\Delta$F1 $= +0.121$
$[+0.067, +0.173]$, $p < 0.001$, at 1.26 model calls against 3.80 and a median
of 4.5\,s against 35.4\,s. Removing the component the brief lists first is
worth twelve F1 points on the harder dataset, at a third of the calls and an
eighth of the wall clock.

That row is also the only configuration anywhere in this grid that
significantly beats a baseline on answer quality. Against \texttt{self\_ask}
it scores $\Delta$F1 $= +0.078$ $[+0.021, +0.135]$ --- at 1.26 model calls
against 1.24, a median of 4.5\,s against 13.2\,s, and a citation grounding of
0.900 against 0.000. Cheaper, faster, more accurate and auditable, all at
once. The report's headline --- that the agentic system trades accuracy for
accountability --- is true of \texttt{agentic\_full} and false of this
ablation of it, which gives up nothing.

The configuration deserves a name it does not have. With the Planner removed
the plan is one identity node, so the system issues a single query exactly as
\texttt{naive\_rag} does; and \texttt{naive\_rag} scores 0.152 and 0.200 on
the same questions. The exact-match gap between two systems that
each ask one question is everything else in the stack: graph evidence beside
the retrieved passages, sentence-granular pooling, synthesis constrained to
cite what it used, and a verification pass. The agentic machinery earns its
cost. The decomposition was spending it.

Across both datasets the direction is consistent and grows with difficulty:
$+0.033$ and not significant on two-hop questions, $+0.121$ at $p < 0.001$ on
longer chains. That ordering rules out the dilution explanation, which
predicts the opposite --- 2WikiMultihopQA's rankings are the worse of the two,
Recall@10 0.727 against 0.876, so a cost that came from diluting a good
ranking should have fallen there. What grows with chain length is the number
of sub-answers an 8B model must compose across. The Planner's failure is not
that it writes bad sub-queries; it is that it sets its own synthesiser a task
that model cannot finish.
...
2WikiMultihopQA was where the case for the component had to be made, and it is
not made there either. The same ablation over its 250 questions gives an exact
match of 0.256 and F1 of 0.285 against 0.224 and 0.267, a paired $\Delta$F1 of
$+0.018$ $[-0.015, +0.053]$, $p = 0.252$ --- a second null, pointing if anything the
other way, on the dataset whose chains are longer. The cost is unchanged: tool
calls 16.00 against 8.65 and a median of 35.4\,s against 15.8\,s. Across both
datasets the KG Navigator accounts for roughly half the system's tool calls
and more than half its median latency, and neither evaluation can detect
anything it returns. Chain length was the variable that should have unlocked
it, and lengthening the chains changed nothing.
...
split rather than a subsample. The grid is complete: nine configurations on
two datasets, eighteen of eighteen cells, every one over the same frozen 250
questions. All three components are therefore measured twice, and two of the
three second measurements changed the reading: the verifier's HotpotQA gain
vanished, and the planner's HotpotQA null became a significant twelve-point
improvement from deleting it. Only the graph read the same way twice.
A component that looked significant on HotpotQA turned out to do nothing on
2WikiMultihopQA, and one that looked like a harmless null there turned out to
be doing measurable damage. The graph is the one component whose reading did
not change, a null both times --- and given what happened to the other two,
the only one that can now be called settled.

\textbf{The runs are not exactly reproducible, and the report has an error bar
it did not report.} Every interval in this document is a paired bootstrap over
questions. That answers one question --- what if a different 250 had been
sampled --- and is silent about another: what does the same system do on the
same questions when it is simply run again. With a decoding temperature of
0.0 and a fixed seed the intended answer was ``nothing''.

\input{../results/tables/replication.tex}

Table~\ref{tab:replication-drift} shows it is not nothing.
\texttt{agentic\_no\_verifier} was re-run on the identical HotpotQA slice ---
a configuration with no verifier, and therefore no feedback loop whose
stochasticity could be blamed --- and exact match moved by $0.012$, F1 by
$0.014$, with 15 of the 250 answers changing. Thirteen of those fifteen also
drew a \emph{different evidence pool}, which locates the divergence in
retrieval rather than in generation.

The cause is recorded in the runs' own metadata. \texttt{meta.json} stores
\texttt{seeding.pythonhashseed}: it is \texttt{null} for the evaluated grid
and \texttt{0} for the re-run. Python randomises string hashing per process
unless that variable is set, the evidence pool is assembled from sets, and set
iteration order follows the hash. The CLI sets the variable; the harness
invoked directly inherits whatever the environment had, which for the
evaluated grid was nothing. Every number in this report therefore comes from
one execution of a pipeline whose retrieval order was not pinned.

The consequence is stated rather than buried, because it changes how three
rows should be read. An effect smaller than roughly $0.014$ F1 is not
established by a single execution. That leaves the planner ablation on
2WikiMultihopQA ($+0.121$) and the verifier ablation on HotpotQA ($-0.037$)
comfortably clear of the drift --- nine times and two and a half times it
respectively --- and puts the knowledge graph's $-0.009$ and $+0.018$, and the
verifier's $-0.001$ on 2WikiMultihopQA, inside it. Those three were reported
as nulls and remain nulls; what this rules out is reading any of them as a
small real effect in either direction. It also means the confidence intervals
elsewhere are narrower than the true uncertainty, since they hold the
execution fixed. Pinning \texttt{PYTHONHASHSEED} in the harness and re-running
the grid is a half-day of compute and the first thing a continuation of this
work should do.
...
\texttt{agentic\_full} re-planned, all 118 had at least one cycle-0 identifier
reused by a later plan and so lost that identifier's cycle-0 results, and 92
end with at least one entry in \texttt{retrieved} whose key the final plan does
not use --- a result carried over from a cycle that is no longer current. Two consequences follow. The retrieval metrics reported for
